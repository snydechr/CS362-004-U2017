...
int main()
{
	...
	while(caseCounter <= 100)
	{
		...
		//configure the random player as if they were the first player in the game. See dominion.c for example of drawing cards
		...
		for(handCounter = 0; handCounter < 5; handCounter++)
		{
			if(handCounter != randCardIdx)
			{
				game1.hand[randPlayerIdx][handCounter] = rand() % 26;
			}
		}
		game1.handCount[randPlayerIdx] = 5;
		...
		//now play adventurer
		//
		//adventurer_action(&game1, randCardIdx, randPlayerIdx); //This is my function call
		adventurerfunc(drawntreasure, &game1, randPlayerIdx, randCardIdx, temphand);
