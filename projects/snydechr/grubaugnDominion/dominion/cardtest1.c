...
int main()
{
	...
	for(numPlayers = 2; numPlayers < (MAX_PLAYERS+1); numPlayers++)
	{
		for(currentPlayer = 0; currentPlayer < numPlayers; currentPlayer++)
		{
			...
			//now play smithy
			//
			//smithy_action(&game1, 3, currentPlayer);	//Chris's version
			smithyfunc(currentPlayer, &game1, 3);
			...
			if(game1.hand[currentPlayer][3] = smithy)
			{
				caseSuccess--;
				printf("smithy_action(): FAIL for discard card \n");
			}
			else
			{
				caseSuccess++;
				printf("smithy_action(): PASS for discard card \n");
			}
