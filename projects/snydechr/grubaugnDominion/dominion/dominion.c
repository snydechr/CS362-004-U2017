...
int comparePlayerState(struct gameState *game1, struct gameState *game2, int player)
{
	...
	if(game1->deckCount[player] != game2->deckCount[player])
	{
		...
		printf("original deck count for player %d: %d\n", player,  game2->deckCount[player]);
		printf("updated deck count for player %d: %d\n", player,  game1->deckCount[player]);
...
void cardName(int cardIndex, char* currentCard)
{
	//char currentCard[50];
	...
	switch( cardIndex ) 
	{
		case curse:		//card 1
		  ...
		  strncpy(currentCard, "curse", sizeof(currentCard));
		  break;
		case estate:	//card 2
